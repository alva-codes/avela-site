		...
		<div className={`${ rootClassnames } container mb-300 md:mb-0`}>
			{formState === 'initial' && (
				<div className={'min-h-[calc(100vh-86px)] pt-180 md:grid grid-cols-12 gap-30'}>
					<h1 className={'typo-h1 font-light text-center mb-170 md:mb-80 md:text-60 col-span-12'}>{headlineCfd}</h1>
					...
					&& (wpFields.map(field => {
						...
						<div className={'mb-50 md:mb-30'}>
							...
							<label>
								...
								/>
								<RenderStaticHTML html={termsOfUseCheckbox} className={`${ ns }__termsOfUse typo-medium-body-semibold text-white inline-block font-normal`} />
			...
			{additionalCopy && (
				<div className={'md:grid grid-cols-12'}>
					<div className={'col-span-12 lg:col-span-10 lg:col-start-2 mt-100 mb-150 md:my-120'}>
						<RenderStaticHTML html={additionalCopy} className={`typo-legal-copy ${ ns }__legal `} />
					</div>
				</div>
			)}
			<div className={'col-span-12 md:flex justify-end'}>
				{informationRow && (
					informationRow.map(info => {
						return (
							<div key={uniqueId(info.label)} className={'ml-60 mb-60 md:ml-0 md:mr-50'}>
								<div className={'typo-medium-body-semibold mb-25'}>{info.label}</div>
								<RenderStaticHTML html={info.info} className={`typo-subhead-03 md:text-24 md:leading-normal`} />
							</div>
						);
					})
				)}
...
export const query = graphql`
	fragment ContactForm on WpPage_Pagecomponents_PageComponents_ContactForm {
		fieldGroupName	
	}
`;
